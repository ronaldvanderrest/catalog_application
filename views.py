...
def fbconnect():
    ...
    print('https://graph.facebook.com/v2.12/me?access_token=%s&fields=name,id,email' % token)
...
def newItem():
    if request.method == 'POST':
        newItemCat_id=session.query(Category).filter_by(name=request.form['category']).one()
        newItem = Item(title=request.form['title'], description=request.form['description'], cat_id=newItemCat_id.id)
        session.add(newItem)
...
def redirectUrlShowItem(cat_id, item_name):
    category = session.query(Category).filter_by(id=cat_id).one()
    return redirect(url_for('showItem', category_name = category.name, item_name = item_name))

@app.route('/catalog/<category_name>/<item_name>/')
def showItem(category_name, item_name):
    category = session.query(Category).filter_by(name=category_name).one()
    item = session.query(Item).filter_by(title=item_name, cat_id=category.id).first()
    if 'username' not in login_session:
        return render_template('publicitemdetails.html', category=category, item = item)
    else:
    ...
    if 'username' not in login_session:
        ...
        if request.method =='POST':
            ...
            if request.form['category']:
                editItemCat_id=session.query(Category).filter_by(name=request.form['category']).one()
                item.cat_id = editItemCat_id.id
            session.add(item)
            session.commit()
            flash('Item successfully edited')
            # to make sure to get new cat_id before setting name again for the redirect
            itemcategory = session.query(Category).filter_by(id=item.cat_id).first()
            return redirect(url_for('showItem', category_name=itemcategory.name, item_name=item.title))
        else:
            return render_template('edititem.html', categories=categories, item = item, itemcategory = itemcategory)
...
def deleteItem(category_name, item_name):
    ...
    if request.method == 'POST':
        ...
        categories = session.query(Category).all()
        itemcategory = session.query(Category).filter_by(id=item.cat_id).first()
    return render_template('deleteitem.html', categories=categories, item = item, itemcategory = itemcategory)
